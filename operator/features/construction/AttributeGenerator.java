 ...
 *
 *  @author Ingo Mierswa
 *  @version $Id: AttributeGenerator.java,v 1.3 2008/05/09 19:22:54 ingomierswa Exp $
 */
		...
		if ((totalMaxNumberOfAttributes < 0) || (exampleSet.getAttributes().size() < totalMaxNumberOfAttributes)) {
			for (int h = 0; h < numberOfNewAttributes; h++) {
				if (random.nextDouble() < pGenerate) {
					...
					FeatureGenerator generator = FeatureGenerator.selectGenerator(exampleSet, generatorList, -1, new String[0], random);
					if (generator != null) {
						generator = generator.newInstance();
						// search necessary features
						Attribute[] args = Tools.getRandomCompatibleAttributes(exampleSet, generator, -1, new String[0], random);
