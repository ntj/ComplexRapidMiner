 ...
 * @version $Id: GeneratingMutation.java,v 1.1 2006/04/14 07:47:17 ingomierswa
 *          Exp $
 */
public class GeneratingMutation extends IndividualOperator {
	...
	private boolean addGeneratedAttribute(AttributeWeightedExampleSet exampleSet, double p) throws Exception {
		if (random.nextDouble() < p) {
			FeatureGenerator generator = FeatureGenerator.selectGenerator(exampleSet, generators, maxDepth, unusedFunctions, random);
			if (generator != null) {
				generator = generator.newInstance();
				Attribute[] args = Tools.getRandomCompatibleAttributes(exampleSet, generator, maxDepth, unusedFunctions, random);
