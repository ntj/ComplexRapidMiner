 ...
 * @see DirectedGGA
 * @author Ingo Mierswa
 * @version $Id: DirectedGeneratingMutation.java,v 1.1 2006/04/14 07:47:17
 *          ingomierswa Exp $
 */
public class DirectedGeneratingMutation extends IndividualOperator {
	...
	public DirectedGeneratingMutation(Attribute[] originalAttributes, double p, List generators, int maxGeneratedAttributes, int maxAddedOriginalAttributes, int maxDepth, String[] unusableFunctions, RandomGenerator random) {
	...
	private void addGeneratedAttribute(AttributeWeightedExampleSet exampleSet) throws GenerationException {
		for (int i = 0; i < maxGeneratedAttributes; i++) {
			if (random.nextDouble() < p) {
				FeatureGenerator generator = FeatureGenerator.selectGenerator(exampleSet, generators, maxDepth, unusableFunctions, random);
				if (generator != null) {
					generator = generator.newInstance();
					Attribute[] args = Tools.getWeightedCompatibleAttributes(exampleSet, generator, maxDepth, unusableFunctions, random);
