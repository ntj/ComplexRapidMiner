...
public class GscEuclideanDistance extends SimilarityAdapter implements ExampleBasedSimilarityMeasure{

	/**
	 * 
	 */
	private static final long serialVersionUID = -4295482154585138602L;
	...
	public double similarity(ConstantArrayValue x, ConstantArrayValue y) {
		...
		double[][] arrayX = x.getValues();
		double[][] arrayY = y.getValues();
		
		double dist = 0.0;
		
		for(int i = 0;i<arrayX.length;i++) {
			
			dist += euclideanDistance(arrayX[i], arrayY[i]);
		}
		
		return dist/arrayX.length;
	}
