 ...
 * @see FourierGGA
 * @author Ingo Mierswa
 * @version $Id: FourierGeneratingMutation.java,v 1.1 2006/04/14 07:47:17
 *          ingomierswa Exp $
 */
public class FourierGeneratingMutation extends IndividualOperator {
		...
		for (int k = 0; k < numberOfConstructed; k++) {
			if (random.nextDouble() < p) {
				...
				if (generator != null) {
					...
					Attribute[] args = Tools.getRandomCompatibleAttributes(exampleSet, generator, maxDepth, unusableFunctions, random);
					...
					while (i.hasNext()) {
						...
						if ((!current.isNominal()) && (current.getConstruction().getDescription().indexOf("sin") == -1)) {
