 ...
 * @version $Id: OneRErrorWeighting.java,v 1.5 2007/07/15 02:01:43 ingomierswa Exp $
 */
public class OneRErrorWeighting extends Operator {
	...
	public IOObject[] apply() throws OperatorException {
		...
		for (Attribute attribute : exampleSet.getAttributes()) {
			...
			AttributeSelectionExampleSet singleAttributeSet = new AttributeSelectionExampleSet(exampleSet, mask);
			// calculating model
			IOContainer ioContainer = new IOContainer(singleAttributeSet);
			Model model = learner.apply(ioContainer).remove(Model.class);
			// applying model
			model.apply(singleAttributeSet);
