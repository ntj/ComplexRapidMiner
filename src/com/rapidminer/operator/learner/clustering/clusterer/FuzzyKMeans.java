...
/**
 * This operator represents a simple implementation of fuzzy k-means.
 * 
 * @author Martin Hahmann
 * @version $Id: FuzzyKMeans.java,v 1.4 2007/07/11 13:43:35 ingomierswa Exp $
 */
public class FuzzyKMeans extends AbstractFlatClusterer {
	...
	public FuzzyKMeans(OperatorDescription description) {
		super(description);
	}
	...
	public ClusterModel createClusterModel(ExampleSet es) throws OperatorException {
		...
		FuzzyKMeansClusterModel bestModel = null;
		for (int iter = 0; iter < maxRuns; iter++) {
			FlatCrispClusterModel oldResult = null;
			// initialize the method
			FuzzyKMeansClusterModel result = new FuzzyKMeansClusterModel(initializeCendroids(es, ids), es, m);
	...
	protected double evaluateClusterModel(ExampleSet es, FuzzyKMeansClusterModel cm) {
		int count = 0;
		double sum = 0.0;
	...
	protected int bestIndex(String id, ExampleSet es, FuzzyKMeansClusterModel cl) {
	...
	protected void recalculateCentroids(ExampleSet es, FuzzyKMeansClusterModel cl) {
		for (int j = 0; j < k; j++) {
