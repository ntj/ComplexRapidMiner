...
/**
 * <p>This operator parses given nominal attributes in order to create date
 * and / or time attributes. The date format can be specified by the 
 * date_format parameter. The old nominal attribute will be removed
 * and replaced by a new date attribute if the corresponding parameter
 * is not set (default).</p>   
 * 
 ...
 * 
 * @author Ingo Mierswa
 * @version $Id: Nominal2Date.java,v 1.7 2008/08/12 12:49:17 tobiasmalbrecht Exp $
 */
public class Nominal2Date extends Operator {
	...
	public IOObject[] apply() throws OperatorException {
		...
		Attribute newAttribute = AttributeFactory.createAttribute(attributeName, valueType);
		exampleSet.getExampleTable().addAttribute(newAttribute);
		exampleSet.getAttributes().addRegular(newAttribute);
		
		SimpleDateFormat parser = new SimpleDateFormat(dateFormat, selectedLocale);
		// parse date as relative to GMT (i.e. ignore time zone)
		parser.setTimeZone(TimeZone.getTimeZone("GMT+00:00"));
		...
		if (!getParameterAsBoolean(PARAMETER_KEEP_OLD_ATTRIBUTE)) {
			...
			oldAttribute.setName(attributeName + "_old");
		}
