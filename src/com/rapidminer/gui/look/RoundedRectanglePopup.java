 ...
 *
 * @author Ingo Mierswa
 * @version $Id: RoundedRectanglePopup.java,v 1.2 2008/05/09 19:23:17 ingomierswa Exp $
 */
public final class RoundedRectanglePopup extends Popup {

	private static final Rectangle rectangle = new Rectangle();

	private static List<RoundedRectanglePopup> allPopups;
	...
	static Popup getInstance(Component owner, Component contents, int x, int y, Popup delegate) {
		...
		synchronized (RoundedRectanglePopup.class) {
			if (allPopups == null) {
				allPopups = new ArrayList<RoundedRectanglePopup>(7);
			}
			if (allPopups.size() > 0) {
				popup = allPopups.remove(0);
			} else {
	...
	private static void addPopupToList(RoundedRectanglePopup popup) {
		synchronized (RoundedRectanglePopup.class) {
			if (allPopups.size() < 7) {
				allPopups.add(popup);
			}
