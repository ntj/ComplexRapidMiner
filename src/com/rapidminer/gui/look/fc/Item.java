 ...
 *
 * @author Ingo Mierswa
 * @version $Id: Item.java,v 1.3 2008/05/09 20:57:26 ingomierswa Exp $
 */
public class Item extends JComponent implements Comparable, MouseListener {
	...
	private class MenuActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getActionCommand().startsWith("Rename the")) {
				...
				int resInt = JOptionPane.showConfirmDialog(getFileChooser(), "Are you sure you want to delete " + getItemName() + "?", "Delete file", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE);
				if (resInt == 0) {
					try {
						res = delete();
					} catch (Exception exp) {
					...
					if (!res) {
						JOptionPane.showConfirmDialog(getFileChooser(), "Unable to delete the selected file.", "Delete file", JOptionPane.PLAIN_MESSAGE, JOptionPane.ERROR_MESSAGE);
					} else {
	...
	public void createPopupMenu() {
		...
		if (this.isDirectory) {
			...
			menuItem.addActionListener(this.menuActionListener);
	...
	private void renameFile() {
		...
		try {
			if (this.response.equals("") || (this.response == null)) {
				JOptionPane.showConfirmDialog(getFileChooser(), "Please insert a valid name.", "Rename the file", JOptionPane.PLAIN_MESSAGE, JOptionPane.ERROR_MESSAGE);
			} else {
				if (this.file.renameTo(new File(this.file.getParentFile(), this.response))) {
					...
					JOptionPane.showConfirmDialog(getFileChooser(), "An error occured while renaming the file.", "Rename the file", JOptionPane.PLAIN_MESSAGE, JOptionPane.ERROR_MESSAGE);
				}
			}
		} catch (Exception exp) {
			// do nothing
