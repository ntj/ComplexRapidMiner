 ...
 * 
 * @author Ingo Mierswa
 * @version $Id: ToggleBreakpointItem.java,v 1.1 2007/05/27 22:02:37 ingomierswa Exp $
 */
public class ToggleBreakpointItem extends JCheckBoxMenuItem implements ActionListener {
	...
	static {
		...
		for (IconSize size : IconSize.values()) {
			UP_ICONS[counter++] = SwingTools.createIcon("icons/" + size.getSize() + "/" + UP_ICON_NAME);
		}
		
		counter = 0;
		for (IconSize size : IconSize.values()) {
			WITHIN_ICONS[counter++] = SwingTools.createIcon("icons/" + size.getSize() + "/" + WITHIN_ICON_NAME);
		}
		
		counter = 0;
		for (IconSize size : IconSize.values()) {
			DOWN_ICONS[counter++] = SwingTools.createIcon("icons/" + size.getSize() + "/" + DOWN_ICON_NAME);
		}
	}
