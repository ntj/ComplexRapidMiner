 ...
 * 
 * @author Ingo Mierswa
 * @version $Id: RunResumeAction.java,v 1.2 2007/06/07 17:12:19 ingomierswa Exp $
 */
public class RunResumeAction extends ConditionalAction {
	...
	static {
		...
		for (IconSize size : IconSize.values()) {
			RUN_ICONS[counter++] = SwingTools.createIcon("icons/" + size.getSize() + "/" + RUN_ICON_NAME);
		}
		
		counter = 0;
		for (IconSize size : IconSize.values()) {
			RESUME_ICONS[counter++] = SwingTools.createIcon("icons/" + size.getSize() + "/" + RESUME_ICON_NAME);
		}
	}
